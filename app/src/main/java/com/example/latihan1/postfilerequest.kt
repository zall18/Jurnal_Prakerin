...
suspend fun postfileRequest(connectionString: String, file: File?, fotoName: String, dataName: MutableList<String>, dataValue: MutableList<String>, token: String?): Result<String> {
        ...
        try {
            ...
            do {
                ...
                for (i in 0 until dataName.size)
                {
                    ...
                    writer.append("Content-Disposition: form-data; name=\"").append(dataName[i]).append("\"")
                        .append(LINE_FEED)
                    writer.append(LINE_FEED)
                    writer.append(dataValue[i]).append(LINE_FEED)
                    writer.flush()
                }

                writer.append("--").append(boundary).append(LINE_FEED)
                writer.append("Content-Disposition: file; name=\"").append(fotoName)
                    .append("\"; filename=\"").append(file?.name.toString()).append("\"").append(LINE_FEED)
